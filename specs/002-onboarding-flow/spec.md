...
- What happens if network connectivity is lost during the final confirmation step? **→ Addressed by FR-021: Network error handling with retry mechanism.**
...
- **FR-001**: The system MUST present a Welcome/Get Started screen to new users who have not completed onboarding, with options to "Create Account" or "Log In" and an optional preview of app benefits (analytics, personalized calories, weekly summaries). This screen serves as the entry point to the onboarding flow. **Note**: The app benefits preview is optional for MVP and may be implemented as a simple text list or card-based layout. If not implemented in MVP, the Welcome screen will focus on the "Get Started" call-to-action.
...
- **FR-010**: The system MUST validate all body measurements and calculated values (body fat percentage, lean body mass) to ensure they fall within physiologically reasonable ranges, displaying clear error messages for invalid inputs. **Validation bounds**: Age must be between 13-120 years, weight must be between 20-500 kg (44-1100 lbs), height must be between 30-300 cm (1-10 feet), body fat percentage must be between 0-50%, as defined in validation constants (see `lib/validation/constants.ts`).
...
- **FR-015**: The system MUST enforce minimum safe calorie thresholds (1200 calories for all adult users, regardless of age or gender) and display warnings when calculated targets fall below safe levels, allowing users to adjust their goal if needed. Warnings MUST be displayed as a prominent, non-blocking alert or banner on the Review screen with: (1) clear messaging explaining the safety concern (e.g., "Your calculated daily calories are below the recommended minimum of 1,200 calories"), (2) an explanation of potential health risks, (3) options to adjust the goal phase (e.g., switch from Aggressive to Moderate) or proceed with acknowledgment. The warning MUST not prevent submission but should require explicit user acknowledgment if proceeding with unsafe values. **Acknowledgment mechanism**: Users must check a checkbox (e.g., "I understand the risks and want to proceed") or tap an "Acknowledge and Continue" button before the final confirmation button becomes enabled when calories are below 1200.
- **FR-016**: The system MUST present a Review & Confirm Plan screen that displays the calculated daily calorie goal, protein target, expected timeline to reach the goal, and start date before final confirmation. **Timeline calculation**: For `goalType = 'target_weight'`, calculate `expectedTimelineWeeks = Math.ceil((currentWeight - goalValue) / (weeklyChangeRate))` where `weeklyChangeRate` is estimated based on goal phase (Slow: ~0.25 kg/week, Moderate: ~0.5 kg/week, Aggressive: ~0.75 kg/week, Maintenance: N/A). For `goalType = 'weekly_change'`, calculate `expectedTimelineWeeks = Math.ceil((currentWeight - estimatedTargetWeight) / Math.abs(goalValue))` where `estimatedTargetWeight` is derived from the user's goal phase and current body composition. If timeline cannot be calculated, display "Timeline will be calculated after plan starts" or similar message.
...
- **UserAccount** (existing entity, to be extended): The UserAccount entity will include an `onboardingCompleted` boolean field to track whether a user has completed the onboarding flow. See `data-model.md` for complete field definition and validation rules. The field is `false` if onboarding is incomplete, and set to `true` when the user finishes onboarding. The actual completion timestamp is stored in `OnboardingProfile.completedAt` for audit trail purposes. The field is checked to determine whether to show the onboarding flow or redirect to the main app.
